...
class SettingsFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        instructions = (
            "Avisos:\n"
            "Ao apertar o botão de salvar, por gentileza reiniciar a aplicação"
        )
        ttk.Label(main_frame, text=instructions, justify=tk.LEFT).pack(pady=10, anchor=tk.W)
        ...
        self.port = ttk.Entry(form_frame, width=40)
        self.port.grid(row=3, column=1, padx=5, pady=5)
        self.port.insert(0, self.config['LDAP'].get('PORT', ''))
    ...
    def on_save(self):
        ...
        self.config['LDAP']['PORT'] = self.port.get()
        self.save_config()
    
    def test_connection(self):
        messagebox.showinfo("Info", "Funcionalidade não implementada")